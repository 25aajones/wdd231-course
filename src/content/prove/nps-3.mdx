...
```
In this case the `resourcePath` is `alerts`, and the parameters are `?parkCode=yell&limit=10`. The "?" is a special character in URLs. It tells the browser that everything after it is a URL Search parameter. The parameters are key/value pairs. The key is the name of the parameter, and the value is the value of the parameter. In this case we have two parameters: `parkCode` and `limit`. The `parkCode` is set to `yell` and the `limit` is set to `10`. Multiple parameters are separated by an `&` character.
...
We have been writing our Javascript in a very modular form.  Breaking up our work into modules allows us to be more organized and write more re-usable code. We can get some of the same benefits with CSS. If you look at the current CSS file you can see that it is already organized somewhat. There are comments that try to indicate what the CSS is doing. Based on those comments, one way we could categorize our CSS is the following:
...
> A "block" is just a portion of our page that is somewhat self-contained.  You might be saying "well wouldn't the header and footer just be more blocks then?" And the answer is technically "yes".  Sometimes as our blocks get more complex they warrent their own file...and sometimes we might group the styles for smaller, simpler blocks together. If this seems a bit arbitrary...it is. 😁
> You may not end up with much in `blocks.css` yet...but we will add more.

Let's create a CSS file for each of those sections. Just to help our organization let's first create a folder in `css` called `partials`. Create your files in there. As you copy/paste the actual CSS out of the `style.css` file replace the lines with an `import`. At the end `style.css` will be very short and look something like this:
...
> Why didn't I put `home.css` in the partials folder? There is a reason why that one is getting treated a bit different...but it will be explained later.

Now when you need to make a change to anything in the CSS for the header...you know exactly where to find it! Having your CSS broken up like this also helps avoid conflicts when working in a team. See the following for more info: [Organizing your CSS](https://byui-cit.github.io/learning-modules/modules/css/@import/)

Test your page to make sure nothing broke after moving everything around.

## **07** Add environment variables to Netlify
